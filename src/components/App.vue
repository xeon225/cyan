<template>
  <div class="flex-container top left">
    <div class="btn-nav pos-f top5 right10 bg-white clearfix" @click="showNav = !showNav">
      <div class="icon-Cyan pos-r" style="font-size:2em">
        <div style="font-size:1em" class="pos-a" :class="'path'+item" v-for="item in data.logo"></div>
      </div>
    </div>
    <div class="nav borderr fontsNav pos-f top0 bottom0 left0" :class="showNav && 'show-nav'">
      <div class="padding20">
        <a href="#!" class="flex-container left text-darker">
          <!-- <svg width="40" height="40" viewBox="0,0,96,96">
            <path :d="data.logo" style="fill:#409eff"></path>
          </svg> -->
          <div class="icon-Cyan pos-r" style="width:40px;height:40px;">
            <div style="font-size:40px;left:0.92578125em" class="pos-a" :class="'path'+item" v-for="item in data.logo"></div>
          </div>
    ...
    <div class="flex1 content flex-container-col" style="min-height:100%;">
